...
1. **Brief the activity, not the room.** 'A place for forty people to eat that empties in twenty minutes' designs better than 'a canteen'. Rooms are a solution; activities are the requirement.
2. **Put an area against everything.** An unpriced schedule is a wish list. Areas are what make the budget conversation possible.
3. **Test the budget early and bluntly.** Total area at a realistic rate per square metre, against the stated budget. If it does not work, say so in the brief rather than discovering it at tender.
4. **Map adjacencies as a matrix.** Must-be-near, must-not-be-near, and the routes people take between them. This is where the plan actually comes from.
5. **Name constraints before they become surprises.** Planning context, rights of light, access, ground conditions, services capacity, listed status.
6. **Write the exclusions down and get them agreed.** The scope you did not include is only excluded if it is on paper.

## Deeper Material

- **[`references/worked-example.md`](references/worked-example.md)** — a community hall whose budget is £461k short of its own accommodation schedule, found before anyone drew a line. Read it when the shape of a good output is unclear, or to calibrate how specific the entries should be.
...
**Budget check:** [y] m² × [rate]/m² = **[construction cost estimate]** vs stated budget **[amount]** → [aligned / gap of [amount]]
**If there is a gap:** [the options — reduce area, change specification, phase, or increase budget]
