...
The first open model distilled from a skills library: a LoRA over Qwen2.5-7B-Instruct trained on 616 pairs derived from the 466 skills of [pm-claude-skills](https://github.com/mohitagw15856/pm-claude-skills) — teaching **skill routing** (which professional discipline a task needs, and why) and **structural discipline** (the output skeleton + self-verification bar that skill enforces).

**What it is:** a fast local router and structure-setter for professional tasks.
**What it is not:** a replacement for running the actual skills with a frontier model — 616 pairs teach the shape of judgment, not its depth. Stated plainly so nobody has to discover it.
