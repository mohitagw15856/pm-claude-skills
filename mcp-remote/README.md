...
Body `{"prompt":"…"}` → `{ skill, pack, confidence, probability, auto, alternatives, ms }` from two typed Choice calls (pack → skill) against a calibrated decision model. `GET /route` says whether it is on; `/route/badge` is a shields.io endpoint. Served through the **Workers AI binding** (`[ai] binding = "AI"`, model `typesafe/jev`) — no key needed. A `JEV_API_KEY` secret (TypeSafe `sk-…` or Vercel AI Gateway `vck_…`) overrides the provider. The same path powers the input guard on `/try` (injection + PII, fail-open). Code: `src/jev.js`; library side: [`integrations/jev/`](../integrations/jev/).
