...
| [`suggest-skill-jev.sh`](suggest-skill-jev.sh) 🆕 | `UserPromptSubmit` | The same nudge, chosen by **one typed decision call** (pack → skill, with confidence) instead of keyword overlap. Falls back to `suggest-skill.sh` without `JEV_API_KEY`, on timeout, or on any error — never blocks a prompt. See [`integrations/jev/`](../integrations/jev/). |
