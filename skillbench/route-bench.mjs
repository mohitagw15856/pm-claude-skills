...
export async function runMethod(method, list, catalog, { transport } = {}) {
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  ...
  const wanted = arg('methods', configured() ? 'keyword,jev-2s,jev-flat' : 'keyword').split(',');
  const rows = []; let note = '';
  for (const m of wanted) {
    if (m !== 'keyword' && !configured()) { note = `_Model rows (${m}) pending: set JEV_API_KEY and re-run._`; continue; }
    const r = await runMethod(m, list, catalog); rows.push(r);
